...
- **Grid**: Uses `compactType={null}` + `preventCollision={true}` for free-placement in both Developer and Viewer modes. All 8 resize handles enabled.
- **Export/Import**: `.ytics` zip files containing `dashboard.json` (full dashboard object with pages, layout, theme, model positions, selection field names) and dataset CSVs. `exportDashboard(datasets, dashboard, selections)` takes selections as third arg. Export is enabled whenever there are datasets or widgets (either suffices).
- **Selections**: `state.selections` is `{ [fieldName]: string[] }`. Empty array `[]` means "all" (pane visible, no filter). Actions: `SET_SELECTION`, `TOGGLE_SELECTION`, `CLEAR_SELECTION` (reset to all), `REMOVE_SELECTION` (delete pane), `CLEAR_ALL_SELECTIONS` (reset all to all). Selection field names are persisted in `.ytics` exports via `selectionFields` array.
...
- **Trend lines**: `widget.showRegression` (boolean) enables trend lines on Line Chart and Scatter Plot. `widget.regressionType`: `'linear'`, `'polynomial'`, `'logarithmic'`, `'exponential'`. `widget.polynomialDegree` (2–6, default 2). Shared `drawTrendLine()` helper in `chartHelpers.js`. Fitting functions in `dataUtils.js`: `linearRegression`, `polynomialRegression`, `logarithmicRegression`, `exponentialRegression`. All trend lines clipped to chart area via SVG clipPath.
- **Log scale**: `widget.useLogScale` (boolean) enables logarithmic Y axis. Supported by Line, Scatter, Combo, Histogram, BoxPlot, Violin, Waterfall charts. Uses shared `makeValueScale(useLog, domain, range)` in `chartHelpers.js`.
- **GeoMap color modes**: `widget.geoColorMode`: `'gradient'` (valueField, sequential scale) or `'categorical'` (colorField, palette). Mutually exclusive. `widget.tooltipMeasures` (array of field names) for tooltip-only numeric measures.
